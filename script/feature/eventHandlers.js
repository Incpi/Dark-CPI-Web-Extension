sap.ui.define(["powerTraceHandler", "constants", "codeEditor", "common", "link", "apiCall", "model", "ExToast", "sap/m/Title", "sap/m/Button", "sap/m/VBox", "sap/m/HBox", "sap/m/Text", "sap/m/IconTabBar", "sap/m/IconTabFilter", "sap/m/Panel", "sap/ui/core/BusyIndicator", "RequestQueue"], function (powerTraceHandler, constants, CustomCodeEditor, common, link, apiCall, Model, ExToast, Title, Button, VBox, HBox, Text, IconTabBar, IconTabFilter, Panel, BusyIndicator, RequestQueue) {
  "use strict";
    ...
    try {
      ...
      for (const item of entriesList.d.results.sort((a, b) => (a.MessageStoreId.toLowerCase() > b.MessageStoreId.toLowerCase() ? 1 : -1))) {
        ...
        apiCall
          ...
          .then((traceResponse) =>
            propertiesTab.addContent(
              common.createTableFromSection({
                apiResponse: JSON.parse(traceResponse).d.results.reduce(
                  (acc, e) => ({
                    ...acc,
                    [e.Name]: e.Value,
                  }),
  ...
  async function createLogTable(messageId) {
    try {
      ...
      const sections = [
        ...
        {
          title: "Integration Flow Information",
          rows: {
      ...
      if (messages.d.results.length > 0) {
        ...
        if (lastID !== newLastID) {
          ...
          Object.entries(groupedMessages).forEach(([date, logs]) => {
            ...
            const buttonConfigs = [
              {
                tooltip: "show log viewer on this page",
                      ...
                      new IconTabBar({
                        ...
                        items: [
                          new IconTabFilter({
                            key: "info",
                            text: "Info",
                            content: await createLogTable(log.MessageGuid),
                          }),
                          new IconTabFilter({
                            key: "runlogs",
                            text: "Run Logs",
                            content: await createRunLogsContent(log.MessageGuid),
                          }),
                          new IconTabFilter({
                            key: "persist",
                            text: "Persist",
                            content: await createPersistLogsContent(log.MessageGuid),
                          }),
                        ],
                      }),
                    ],
                    endButton: { label: "Close" },
  ...
  const infoButton = async (event) => {
    try {
      const busy = new sap.m.BusyIndicator({ visible: true });

      const dialog = new Model({
        title: "Dark CPI Panel - Information",
        content: [busy], // Initially show the busy indicator
        verticalScrolling: true,
      });

      dialog.open(); // Ensure the dialog is opened immediately
      ...
      if (!traceResponse) {
        ...
        dialog.addContent(new sap.m.Text({ text: "Failed to fetch data." }).addStyleClass("sapUiSmallMargin"));
        return;
      }
      ...
      if (!flowId) {
        ...
        dialog.removeAllContent();
        dialog.addContent(new sap.m.Text({ text: "No matching artifact found." }).addStyleClass("sapUiSmallMargin"));
        return;
      }
      ...
      const { id, tenantId, ...apiResponse } = Array.from(apidatacollection.querySelectorAll("* > artifactInformation > *"))
      ...
      apiResponse["Active Trace"] = apidatacollection.querySelector("* > logConfiguration >traceActive")?.textContent || "N/A";
      apiResponse["Enabled Trace"] = apidatacollection.querySelector("* > logConfiguration >traceEnabled")?.textContent || "N/A";
      ...
      if (Object.keys(apiResponse).length === 0) {
        ...
        dialog.removeAllContent();
        dialog.addContent(new sap.m.Text({ text: "No data available." }).addStyleClass("sapUiSmallMargin"));
        return;
      }

      // Remove busy indicator and add table content
      ...
      if (Object.keys(endpoint).length > 0) {
        dialog.addContent(
          new VBox({
            items: [
              new Title({ text: "Endpoint Information" }),
              ...Object.entries(endpoint).map(([label, value]) =>
                new HBox({
                  alignItems: "Center",
                  ...
                  items: [
                    ...
                    new Button({
                      icon: "sap-icon://copy",
                      tooltip: "Copy",
                      press: () => common.copyToClipboard(value),
                    }),
                  ],
