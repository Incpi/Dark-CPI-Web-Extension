sap.ui.define(
  ...
  function (
    ...
    return {
      messageButton: async (event) => {
        ...
        popover = new sap.m.Popover(`${constants.prefixId}Popover`, {
          ...
          beginButton: new Button({
            ...
            text: "Off",
            press: powerTraceHandler.hideInlineTrace,
          }),
        });
        let lastID;
        popover.openBy(event.getSource());
        // Open popover dynamically with updated content
        const attachOpen = async () => {
          ...
          if (messages.d.results.length > 0) {
            if (lastID !== messages.d.results[0].MessageGuid + messages.d.results[0].LogTime.split(".")[0] + messages.d.results[0].Status) {
              lastID = messages.d.results[0].MessageGuid + messages.d.results[0].LogTime.split(".")[0] + messages.d.results[0].Status;
              ...
              Object.entries(groupedMessages).forEach(([date, logs]) => {
                ...
                // Define reusable button creation function
                const oButton = ({ icon, tooltip, text, press, pressed, isToggle = false }) => {
                  if (isToggle) {
                ...
                const logsVBox = new VBox({
                  items: logs.map((log) => {
                    ...
                    // Generate additional buttons dynamically based on buttonConfigs
        ...
        await attachOpen();
        intervalId = setInterval(async () => await attachOpen(), 5 * 1000); //5-second update
      },
      infoButton: async (event) => {
        try {
          const dialog = new Model({ title: "Dark CPI Panel - Information", verticalScrolling: true });
          dialog.open();
      ...
      powerTrace,
      inline: (event) => {
        const data = !$.sap.DarkCPI._settings.trace;
        event.getSource().setIcon(`sap-icon://${data ? "show" : "hide"}`);
        $.sap.DarkCPI._settings.trace = data;
      },

      // TEST:async () => {
      // await RequestQueue.setData("username","john_doe");
      // await RequestQueue.setData("theme","dark");
      // console.log("0",await RequestQueue.getData("username"));
      // console.log("1",await RequestQueue.getData("theme"));
      // },
    };
  }
);
