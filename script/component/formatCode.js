sap.ui.define([], function () {
  ...
  function formatXml(sourceXml) {
    try {
      ...
      const xsltDoc = new DOMParser().parseFromString(['<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform">', '  <xsl:strip-space elements="*"/>', '  <xsl:template match="node()|@*">', '    <xsl:copy><xsl:apply-templates select="node()|@*"/></xsl:copy>', "  </xsl:template>", '  <xsl:output method="xml" indent="yes" omit-xml-declaration="no"/>', "</xsl:stylesheet>"].join("\n"), "application/xml");
